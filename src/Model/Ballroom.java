...
Done by					:	Lee Kai Quan,A AMEENUDEEN (111942S), Adeel M. Ateeque(112013Z)
Admin No				:	114173S
...
public class Ballroom
{
	...
	public String CREATE_BALLROOM()
	{
		...
		sqlQuery += "VALUES ('" + this.ballroomName + "', '" + this.ballroomDescription + "', '" + this.ballroomSize + "', " + this.ballroomDiscount + ", " + this.ballroomPrice + ", "
				+ this.ballroomFinalPrice + ", " + this.ballroomHits + ", " + this.facilityID + ", " + this.ballroomAvailability + ")";
	...
	public boolean UPDATE_BALLROOM(String ID)
	{
		...
		sqlQuery = "UPDATE Ballroom SET `ballroomName`='" + this.ballroomName + "', `ballroomDescription`='" + this.ballroomDescription + "', `ballroomSize`='" + this.ballroomSize
				+ "', `ballroomDiscount`=" + this.ballroomDiscount + ", `ballroomPrice`=" + this.ballroomPrice + ", `ballroomFinalPrice`=" + this.ballroomFinalPrice + ", `ballroomAvailability`="
	...
	public DefaultTableModel RETRIEVE_BALLROOM()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				model.addRow(new Object[] { rs.getString("ballroomID"), rs.getString("ballroomName"), rs.getString("ballroomSize"), rs.getString("ballroomFinalPrice"),
						rs.getString("ballroomDescription") });
			}
		}
		catch (Exception e)
		{
	...
	/*******************************************************
	 * Author : A Ameenudeen(111942S) Method Name : GET_BALLROOM_DETAILS Input Parameter : String eventName Return : ArrayList<String> Purpose : To
	 * get the ballroom details
	 * *****************************************************/
	public ArrayList<Ballroom> GET_BALLROOM_DETAILS(String eventName)
	{
		...
		dbQuery = "SELECT b.ballroomName,b.ballroomFinalPrice FROM Event e INNER JOIN Package p On e.packageID=p.packageID INNER JOIN Ballroom b On p.ballroomID=b.ballroomID  WHERE e.eventName="
				+ "'" + eventName + "'";
		try
		{
