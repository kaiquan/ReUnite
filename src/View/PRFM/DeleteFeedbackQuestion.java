...
public class DeleteFeedbackQuestion {
	...
	JFrame getJFrame() {
		if (jFrame == null) {
			jFrame = new JFrame();
			jFrame.setTitle("Delete Feedback Question #" + fq.getCode().get(0).toString());
	...
	private JButton getDeleteButton() {
		if (deleteButton == null) {
			...
			deleteButton.setBounds(new Rectangle(320, 180, 100, 20));
			deleteButton.setText("Delete");
			deleteButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int option;
					if (!getFormTextField().getText().equals("NIL")){
						option = JOptionPane.showConfirmDialog(getJFrame(), "Do you really want to delete this feedback question with the implicated feedback form(s)?", "Message", JOptionPane.YES_NO_OPTION);	
					}
					else{
						option = JOptionPane.showConfirmDialog(getJFrame(), "Do you really want to delete this feedback question?", "Message", JOptionPane.YES_NO_OPTION);	
					}
					...
					if (option == JOptionPane.YES_OPTION){
						...
						if (!getFormTextField().getText().equals("NIL")){
							...
							while (valid && secondValid && i < ff.getCode().size()){
								EventForm ef = new EventForm();
								ef = ffController.processEventFormSearchTerm(ff.getCode().get(i), 0);
								
								if (!ef.getCode().isEmpty()){
									secondValid = ffController.processDeleteEventForm(ff.getCode().get(i), null);
								}
								
								valid = ffController.processDelete(ff.getCode().get(i), 0);
								i++;
							}
							...
							if (fqController.processDelete(code) && valid && secondValid){
	...
	public static void main(String[] args){
		DeleteFeedbackQuestion delete = new DeleteFeedbackQuestion();
		delete.getJFrame().setVisible(true);
	}
	
	public DeleteFeedbackQuestion(){
		
	}
	
	public DeleteFeedbackQuestion(int code){
		this.code = code;
		AdministrateFeedbackQuestionController fqController = new AdministrateFeedbackQuestionController();
